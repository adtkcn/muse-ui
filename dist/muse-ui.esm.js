...
function getColor(color) {
  ...
  return colors.indexOf(color) !== -1 ? Colors[color] : color;
}
...
function bindScroll(el, binding) {
  ...
  if (el._onScroll && target !== el._onScroll.target) unbind(el);
  ...
  return {
    ...
    methods: {
      start: function start(event) {
        ...
        if (this.ripple && this.$refs.ripple && (event.type !== 'mousedown' || event.button === 0)) {
...
var Grid = {
  install: function install(Vue) {
    Vue.component(Container$1.name, Container$1);
...
export { Alert, AppBar, AutoComplete, Avatar, Badge, BottomNav, BottomSheet, Breadcrumbs, Button, Card, Carousel, Checkbox, Chip, Colors, DataTable, DateInput, Dialog, Divider, Drawer, ExpansionPanel, Form, Grid, GridList, Helpers, Icon, List, LoadMore, Menu, Pagination, Paper, Picker, Popover, Progress, Radio, Select, Picker$1 as SlidePicker, Slider, Snackbar, Stepper, SubHeader, Switch, Tabs, TextField, Tooltip, install, theme, version };
